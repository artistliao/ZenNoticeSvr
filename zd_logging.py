...
handler2 = logging.handlers.RotatingFileHandler("./logs/zen_notice_" + time.strftime("%m%d%H%M", time.localtime()) + ".log", mode="a", maxBytes=1024*1024, backupCount=0, encoding="utf-8")
...
console_formatter = colorlog.ColoredFormatter(
	fmt='%(log_color)s[%(asctime)s.%(msecs)03d] [%(thread)d] [%(lineno)d] [%(levelname)s]- [%(message)s]',
	datefmt='%Y-%m-%d %H:%M:%S',
